...
function getEndereco(req, res, next) {
  ...
  db.one('SELECT * FROM endereco3 WHERE id = $1', id)
    .then(function (data) {
      res.status(200)
        .json({
          status: 'success',
...
function getEnderecoUser(req, res, next) {
  var id = parseInt(req.params.id);
  db.any('SELECT * FROM endereco3 WHERE user_id = $1', id)
    .then(function (data) {
      res.status(200)
        .json({
          status: 'success',
...
function createEndereco(req, res, next) {
  ...
  db.none('INSERT INTO public.endereco3(logradouro,numero,bairro,cidade,cep,uf,user_id,complemento)' +
  'VALUES (${logradouro}, ${numero}, ${bairro}, ${cep}, ${cidade}, ${uf},${user},${complemento})',
  req.body)
    .then(function () {
      res.status(200)
        .json({
...
function updateEndereco(req, res, next) {
  db.none('UPDATE public.endereco3 SET logradouro=$1, numero=$2, bairro=$3, cidade=$4, cep=$5, uf=$6, complemento=$7 where id=$8',
...
function removeEndereco(req, res, next) {
  var id = parseInt(req.params.id);
  db.result('DELETE FROM public.endereco3 WHERE id = $1', id)
